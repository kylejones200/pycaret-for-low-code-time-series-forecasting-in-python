...
class ARCHVisualizer:
    ...
    def generate_data(self):
        ...
        self.data = pd.DataFrame(
            {"returns": self.returns, "volatility": self.volatility}
        )
    ...
    def create_expanding_window_animation(self, n_splits=3):
        ...
        def animate(i):
            ...
            if test_end <= total_points:
                ...
                ax.axvspan(
                    self.dates[0], self.dates[train_end - 1], alpha=0.1, color="blue"
                )
                ax.axvspan(
        ...
        anim = FuncAnimation(
            fig, animate, frames=15 * n_splits, interval=500, repeat=True
        )
        return anim
...
class TimeSeriesFeatureVisualizer:
    ...
    def create_animation(self):
        ...
        anim = FuncAnimation(
            fig, animate, frames=len(self.df) - self.window, interval=200, repeat=True
        )
        return anim
